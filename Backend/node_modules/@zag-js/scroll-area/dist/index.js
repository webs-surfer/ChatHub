  ...
  implementations: {
    actions: {
      ...
      setThumbSize({ context, scope, prop }) {
        ...
        if (!scrollbarXHidden) {
          if (prop("dir") === "rtl") {
            scrollLeftFromStart = utils.clampValue(-scrollLeft, 0, maxScrollLeft);
          } else {
            scrollLeftFromStart = utils.clampValue(scrollLeft, 0, maxScrollLeft);
          }
          scrollLeftFromEnd = maxScrollLeft - scrollLeftFromStart;
        }
        const scrollTopFromStart = !scrollbarYHidden ? utils.clampValue(scrollTop, 0, maxScrollTop) : 0;
        const scrollTopFromEnd = !scrollbarYHidden ? maxScrollTop - scrollTopFromStart : 0;
        domQuery.setStyleProperty(viewportEl, "--scroll-area-overflow-x-start", `${scrollLeftFromStart}px`);
        domQuery.setStyleProperty(viewportEl, "--scroll-area-overflow-x-end", `${scrollLeftFromEnd}px`);
        domQuery.setStyleProperty(viewportEl, "--scroll-area-overflow-y-start", `${scrollTopFromStart}px`);
        domQuery.setStyleProperty(viewportEl, "--scroll-area-overflow-y-end", `${scrollTopFromEnd}px`);
      },
