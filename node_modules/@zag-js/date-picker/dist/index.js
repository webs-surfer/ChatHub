...
var getVisibleRangeText = core.memo(
  ...
  ([view], opts) => {
    ...
    if (view === "year") {
      const years = dateUtils.getDecadeRange(startValue.year, { strict: true });
      const start2 = years.at(0).toString();
      const end2 = years.at(-1).toString();
      return { start: start2, end: end2, formatted: `${start2} - ${end2}` };
    }
    if (view === "month") {
      const formatter2 = new date.DateFormatter(locale, { year: "numeric", timeZone });
    ...
    const formatter = new date.DateFormatter(locale, { month: "long", year: "numeric", timeZone });
...
function connect(service, normalize) {
  ...
  function getDayTableCellState(props2) {
    ...
    const isSelected = selectedValue.some((date) => dateUtils.isDateEqual(value, date));
    const cellState = {
      invalid: dateUtils.isDateOutsideRange(value, min, max),
      disabled: disabled2 || !outsideDaySelectable && isOutsideRange || dateUtils.isDateOutsideRange(value, min, max) || // Disable unselected dates when max is reached in multiple selection mode
  ...
  return {
    ...
    getWeekNumber(week) {
      ...
      return firstDay ? dateUtils.getWeekOfYear(firstDay, locale) : 0;
    },
    ...
    setTime(time, index = 0) {
      ...
      if (!("hour" in dateValue)) {
        dateValue = date.toCalendarDateTime(dateValue);
      }
      ...
      values[index] = dateUtils.constrainValue(dateValue, min, max);
    ...
    getYearsGrid(props2 = {}) {
      ...
      const years = dateUtils.getDecadeRange(startValue.year, { strict: true }).map((year) => ({
        label: year.toString(),
        value: year,
        disabled: !utils.isValueWithinRange(year, min?.year, max?.year)
      }));
      return utils.chunk(years, columns);
    },
    getDecade() {
      const years = dateUtils.getDecadeRange(startValue.year, { strict: true });
    ...
    getWeekNumberCellProps(props2) {
      ...
      const weekNumber = week[0] ? dateUtils.getWeekOfYear(week[0], locale) : 0;
      return normalize.element({
        ...parts.tableCell.attrs,
        role: "rowheader",
  ...
  implementations: {
    guards: {
      ...
      canSelectDate: ({ context, prop, event }) => {
        ...
        const isDeselecting = existingValues.some((date) => dateUtils.isDateEqual(date, currentValue));
    ...
    actions: {
      ...
      toggleSelectedDate(params) {
        ...
        const existingValues = context.get("value");
        const index = existingValues.findIndex((date) => dateUtils.isDateEqual(date, currentValue));
        if (index === -1) {
...
var preserveTime = (existingDate, newDate) => {
  ...
  if (!("hour" in newDate)) {
    if (isZoned) {
      dateWithTime = date.toZoned(date.toCalendarDateTime(newDate), existingDate.timeZone);
    } else {
      dateWithTime = date.toCalendarDateTime(newDate);
    }
  }
